...
if __name__ == "__main__":
    ...
    parser.add_argument("--chunksize",
                        type=int,
                        help="max chunk size (number of files to tokenize at a time - "
                             "smaller should reduce memory use)",
                        default=CHUNKSIZE)
    parser.add_argument("--no_norm",
                        default=False, action="store_true",
                        help="do not normalize difference in p(t|C) by sum p(t|C)")
    parser.add_argument("corpus",
                        help="read corpus from CORPUS_DIR", metavar="CORPUS_DIR")
    parser.add_argument("pairs",
                        metavar='LANG_PAIR', nargs="*",
                        help="language pairs to compute BL weights for")
    ...
    # paths = zip(*items)[2] - changed for Python 3
